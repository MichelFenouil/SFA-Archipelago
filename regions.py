...
def connect_regions(world: SFAWorld) -> None:
    ...
    thorntail_hollow.connect(
        ...
        lambda state: mc.can_explode_bomb_plant(state, world.player) and state.has("Rock Candy", world.player),
    )
    # ice_mountain.connect(sw_water_spout, "Ice Mountain to SnowHorn Wastes",
    # lambda state: state.has("Tricky", world.player))
    # Temp solution to generate roots early
    ice_mountain.connect(
        sw_water_spout,
        "Race down to SnowHorn Wastes",
        lambda state: state.has_all_counts({"Tricky": 1, "Alpine Root": 2}, world.player),
    )
    sw_water_spout.connect(
        sw_entrance,
        "SW - Water Spout to Entrance",
        lambda state: state.has("Alpine Root", world.player, 2),
    )
    sw_entrance.connect(
        sh_sw_transition,
        "Transition to SnowHorn Wastes",
        lambda state: state.has_all(["Scarab Bag (Progressive)", "Tricky"], world.player),
    )
