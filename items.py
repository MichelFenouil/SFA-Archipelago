...
class SFAItemData():
    ...
    @classmethod
    def get_by_id(cls, id: int) -> SFAItemData:
...
STAFF_ITEMS: dict[str, SFAStaffItemData] = {
...
TRICKY_ITEMS: dict[str, SFAItemData] = {
    "Tricky": SFAItemData(10, 0x0846, T2_ADDRESS, SFAItemType.TRICKY, ItemClassification.progression)
}

INVENTORY_ITEMS: dict[str, SFAItemData] = {
    "Scarab Bag (Progressive)": SFAProgressiveItemData(100, 0x0, T2_ADDRESS, SFAItemType.PROGRESSIVE, ItemClassification.progression, [(0x035B, T2_ADDRESS, 1), (0x035C, T2_ADDRESS, 1), (0x035D, T2_ADDRESS, 1)]),
    "Bomb Plant": SFAItemData(101, 0x0, T2_ADDRESS, SFAItemType.INVENTORY, ItemClassification.progression),
    "Alpine Root": SFACountItemData(102, 0x0030, T2_ADDRESS, SFAItemType.INVENTORY, ItemClassification.progression, max_count=2, bit_size=3),
}

SHOP_ITEMS: dict[str, SFAItemData] = {
    "Rock Candy": SFAItemData(200, 0x035F, T2_ADDRESS, SFAItemType.SHOP_PROGRESSION, ItemClassification.progression),
    ...
    "Firefly Lantern": SFAItemData(204, 0x0717, T2_ADDRESS, SFAItemType.SHOP_PROGRESSION, ItemClassification.progression),
    "Snowhorn Artifact": SFAItemData(205, 0x0060, T2_ADDRESS, SFAItemType.SHOP_PROGRESSION, ItemClassification.progression),
}

ALL_PROGRESSION_ITEMS: dict[str, SFAItemData] = {
    **STAFF_ITEMS,
    **INVENTORY_ITEMS,
    **SHOP_ITEMS,
    **TRICKY_ITEMS,
...
USEFUL_ITEMS: dict[str, SFAItemData] = {
    "MP Upgrade": SFACountItemData(300, 0x0, PLAYER_MAX_MP, SFAItemType.USEFUL, ItemClassification.useful, 6, 8, 25, 25),
    "HP Upgrade": SFACountItemData(301, 0x0, PLAYER_MAX_HP, SFAItemType.USEFUL, ItemClassification.useful, 6, 8, 4, 12),
}

FILLER_ITEMS: dict[str, SFAItemData] = {
    "Junk": SFAItemData(1000, 0x0, 0, SFAItemType.FILLER, ItemClassification.filler),
    "Fuel Cell": SFACountItemData(1001, 0x0935, T2_ADDRESS, SFAItemType.FILLER, ItemClassification.filler, max_count=255, bit_size=8),
...
def create_all_items(world: SFAWorld) -> None:
    ...
    for name, data in ALL_PROGRESSION_ITEMS.items():
        if name == "Staff" or name == "Bomb Plant":
            continue
        if isinstance(data, SFACountItemData):
            for _ in range(data.max_count):
